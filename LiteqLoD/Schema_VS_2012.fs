...
let get = JsonExtensions.GetProperty
let tryGet = JsonExtensions.TryGetProperty
...
type HTTPSchema() =
    interface ISchemaProvider with
        ...
        member __.GetAllTypesIn typecluster =
            ...
            let x = JsonValue.Load(uri)
            match get(x,"response") with
            | JsonValue.Array response -> [for item in response -> trimUri(asString item)]
        ...
        member __.GetTypeClustersFor ``type`` =
            ...
            match get (JsonValue.Load(uri), "response") with
            | JsonValue.Array response -> [for item in response -> trimUri(asString item)]
        ...
        member __.GetPropertiesAndTypeClusterIn equivalenceClass =
            ...
            | JsonValue.Array response -> Array.toList (response) |> List.map (fun x -> 
                Array.toList( asArray (asArray x).[1] ) |> List.map (fun x -> trimUri(asString x)),
                Array.toList( asArray (asArray x).[0] ) |> List.map (fun x -> trimUri(asString x)))
        ...
        member __.GetAllInstancesIn equivalenceClass =
            ...
            match tryGet (JsonValue.Load(uri), "response") with
            | Some (JsonValue.Array response) -> [for item in response -> trimUri(asString item)]
